...
# read_csv("https://raw.githubusercontent.com/therbootcamp/therbootcamp.github.io/master/_sessions/_data/patient_demo.csv")
```
...
4. Using the following template, load the `trial_act.csv` data into R and store it as a new object called `trial_act` 
...
3. Now use the `pivot_wider()` function to bring the data *back* into the wide format! 
...
CD4_wide %>%
  group_by(arms) %>%
  summarize(cd40_mean = mean(cd40, na.rm = TRUE),
            cd420_mean = mean(cd420, na.rm = TRUE),
            cd496_mean = mean(cd496, na.rm = TRUE),
            n = n())
```

2. Handle the missing values by including `na.rm = TRUE` in the `mean()` functions. 

3. Use `n()` to add another summary showing the number of cases per arm.

4. Now try to calculate the same values, but using the `CD4_long` dataset. How do you need to adjust the code?
...
5. Finally, use `pivot_wider` to change the format of the summarized results to the one obtained from `CD4_wide`. 
```{r}
CD4_long %>%
  group_by(arms, name) %>%
  summarize(mean_val = mean(value, na.rm = TRUE),
